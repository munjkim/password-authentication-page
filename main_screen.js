...
// Function to get Unix timestamp in microseconds
function getUnixTimestampMicroseconds() {
    const now = Date.now(); // Current time in milliseconds
    const performanceNow = performance.now(); // High-resolution time in milliseconds
    const highResolutionTime = performanceNow % 1000; // Sub-millisecond part
    return (now * 1000) + Math.floor(highResolutionTime * 1000);
}
...
document.getElementById('password').addEventListener('keydown', function(event) {
    ...
    // Log keydown event data
    const timestamp = getUnixTimestampMicroseconds();
...
document.getElementById('passwordForm').addEventListener('submit', function(event) {
    ...
    const currentTime = getUnixTimestampMicroseconds();
...
document.getElementById('startButton').addEventListener('click', function() {
    ...
    startTime = getUnixTimestampMicroseconds();

    previousInputTime = startTime; // Initialize previousInputTime with start time
    keyEventData = [];

    intervalId = setInterval(() => {
        const elapsedTime = Math.floor((getUnixTimestampMicroseconds() - startTime) / 1000000); // Convert to seconds
        document.getElementById('elapsedTime').textContent = `Elapsed time: ${elapsedTime}s`;
    }, 1000);
